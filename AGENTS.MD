...
- Fan-out `gh` reads over many runs/checks/IDs: max 4 in flight. Never unbounded `Promise.all`/`allSettled`, `xargs -P0`, or `&` loops; one such census slowed the shared relay for every Mac (2026-09-23).
